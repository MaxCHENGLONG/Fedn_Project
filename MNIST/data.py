...
def split(out_dir="data"):
    ...
    # 加载训练和测试数据集
    train_data = torchvision.datasets.MNIST(root=f"{out_dir}/train", download=True, transform=torchvision.transforms.ToTensor(), train=True)
    test_data = torchvision.datasets.MNIST(root=f"{out_dir}/test", download=True, transform=torchvision.transforms.ToTensor(), train=False)
