...
def get_data(out_dir="data"):
    ...
    if not os.path.exists(f"{out_dir}/train"):
        torchvision.datasets.CIFAR10(root=f"{out_dir}/train", transform=torchvision.transforms.ToTensor(), train=True, download=True)
    if not os.path.exists(f"{out_dir}/test"):
        torchvision.datasets.CIFAR10(root=f"{out_dir}/test", transform=torchvision.transforms.ToTensor(), train=False, download=True)
...
def splitset_by_dirichlet_imbalanced(dataset, num_clients, alpha, imbalance_ratios=None):
    ...
    # Initialize data containers for each client
    ...
    # Adjust client data volume for imbalance
    ...
    # Convert data to tensors - with optimization
    for i in range(num_clients):
        if client_data[i]['x']:
            # Convert lists to numpy arrays first
            x_np = np.array(client_data[i]['x'])
            y_np = np.array(client_data[i]['y'])
            # Convert numpy arrays to tensors
            client_data[i]['x'] = torch.from_numpy(x_np)
            client_data[i]['y'] = torch.from_numpy(y_np)
        else:
